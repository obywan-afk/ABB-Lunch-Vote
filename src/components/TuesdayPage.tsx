...
export function TuesdayPage({ restaurants, language, setLanguage, aiLimited }: TuesdayPageProps) {
  ...
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      ...
      try {
        const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=60.1699&longitude=24.9384&current_weather=true&timezone=Europe/Helsinki&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,weathercode&forecast_days=7')
        if (res.ok) {
          ...
          const weatherData: WeatherInfo = {
            ...
            high: typeof data?.daily?.temperature_2m_max?.[0] === 'number' ? Math.round(data.daily.temperature_2m_max[0]) : undefined,
            low: typeof data?.daily?.temperature_2m_min?.[0] === 'number' ? Math.round(data.daily.temperature_2m_min[0]) : undefined,
            precipitation: typeof data?.daily?.precipitation_probability_max?.[0] === 'number' ? Math.round(data.daily.precipitation_probability_max[0]) : undefined,
            wind: typeof data?.current_weather?.windspeed === 'number' ? Math.round(data.current_weather.windspeed) : undefined,
      ...
      let highlight: DailyHighlight | null = null
      try {
        // Fetch from design-related subreddits
      ...
      if (!highlight) {
        try {
          const quoteRes = await fetch('https://api.quotable.io/random?tags=food|happiness|success')
          if (quoteRes.ok) {
      ...
      if (!isCancelled && highlight) {
        setDailyHighlight(highlight)
      }
